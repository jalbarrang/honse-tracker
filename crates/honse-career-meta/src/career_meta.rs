...
/// The character id behind a trained outfit `card_id`, from the id alone: the
/// leading four digits. `None` when the card id is too short to hold one.
///
/// This is the fallback half of a portrait lookup. The plugin has an outfit
/// catalogue and should ask it first; anything without one — the career viewer
/// reading exported JSON, for instance — has only this, and it is right for
/// every card observed so far.
#[must_use]
pub fn chara_id_from_card_id(card_id: i32) -> Option<i64> {
    card_id
        .to_string()
        .get(0..4)
...
mod tests {
    ...
    #[test]
    fn chara_id_comes_off_the_front_of_the_card_id() {
        assert_eq!(chara_id_from_card_id(100_702), Some(1007));
        assert_eq!(chara_id_from_card_id(101_302), Some(1013));
        // Too short to carry one, so the caller falls back to no portrait
        // rather than to a wrong one.
        assert_eq!(chara_id_from_card_id(102), None);
    }
